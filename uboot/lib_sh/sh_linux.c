		     ...
		     ulong addr, ulong * len_ptr, int verify)
{
	...
	if (data) {
		...
		*LOADER_TYPE = 1;
		*INITRD_START = initrd_start;//(initrd_start - (param - PAGE_OFFSET)) ;	/* passed of offset from memory base */
		*INITRD_SIZE = len;
	} else {
		*LOADER_TYPE = 0;
		*INITRD_START = 0;
	...
	strcpy (COMMAND_LINE, commandline);
		{
			...
			{
				sprintf((char*)Buffer,"hwaddr:%c%c:%c%c:%c%c:%c%c:%c%c:%c%c", MacAddr[0],MacAddr[1],MacAddr[2],MacAddr[3], \
			...
			/* sprintf(Buffer,"hwaddr:%c%c:%c%c:%c%c:%c%c:%c%c:%c%c", MacAddr[0],MacAddr[1],MacAddr[2],MacAddr[3], \
							MacAddr[4],MacAddr[5],MacAddr[6],MacAddr[7],MacAddr[8],MacAddr[9],MacAddr[10],MacAddr[11]);
			*/
			memcpy((void*)(&CMDHeader[i]), Buffer, strlen(Buffer));
